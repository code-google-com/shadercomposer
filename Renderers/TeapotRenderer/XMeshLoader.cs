...
namespace TeapotRenderer
{
    class XLoader : IDisposable
    {
        ...
        public void LoadFile(string filename, out Vector3[] vertexBuffer, out short[] indexBuffer)
        {
            using (var mesh9 = D3D9.Mesh.FromFile(device9, filename, D3D9.MeshFlags.SystemMemory))
            {
                ...
                // Read index buffer
                indexBuffer = new short[mesh9.IndexBuffer.Description.SizeInBytes / 2];

                DataStream indexStream = mesh9.IndexBuffer.Lock(0, mesh9.IndexBuffer.Description.SizeInBytes, D3D9.LockFlags.ReadOnly);

                indexStream.ReadRange(indexBuffer, 0, mesh9.IndexBuffer.Description.SizeInBytes / 2);
